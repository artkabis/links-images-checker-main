...
import { 
  ...
  export const DEFAULT_LINK_OPTIONS = {
    ...
    userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36', // User-Agent plus réaliste
  ...
  export async function checkLink(url, options = {}) {
    ...
    if (SPECIAL_PROTOCOLS.some(protocol => url.toLowerCase().startsWith(protocol))) {
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        ...
        if (opts.checkAnchors && url.includes('#') && result.status === 'success') {
